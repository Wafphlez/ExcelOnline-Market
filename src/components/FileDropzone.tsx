  ...
  if (embedded) {
    return (
      <div
        ...
        className={`rounded border border-dashed px-4 py-4 shadow-eve-inset transition-colors sm:flex sm:items-center sm:justify-between sm:gap-4 ${
          drag
            ? 'border-eve-accent/80 bg-eve-highlight'
            : 'border-eve-border/80 bg-eve-bg/50'
        } ${disabled ? 'pointer-events-none opacity-50' : ''}`}
      >
        <div className="mb-3 flex items-start gap-3 sm:mb-0 sm:min-w-0 sm:flex-1">
          <Upload
            className="h-7 w-7 shrink-0 text-eve-gold/80"
            aria-hidden
          />
          <div className="min-w-0 text-left">
            <p className="text-sm font-semibold text-eve-bright/95">
              Перетащите сюда .xlsx / .xls или выберите файл
            </p>
            <p className="mt-0.5 text-xs text-eve-muted/90">
              Обработка только в браузере, на сервер не уходит
            </p>
          </div>
        </div>
        <label className="block shrink-0 sm:ml-auto">
          <input
            ref={inputRef}
            type="file"
          ...
          <button
            ...
            className="w-full rounded border border-eve-accent/70 bg-eve-accent-muted px-4 py-2 text-xs font-semibold text-eve-accent transition-colors hover:border-eve-accent hover:bg-eve-highlight focus:outline-none focus:ring-2 focus:ring-eve-accent/35 disabled:opacity-50 sm:w-auto"
          >
            Выбрать файл…
          </button>
        </label>
